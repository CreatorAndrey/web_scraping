...
def get_snils(folder):
    ...
    # mn1.progress_bar['value'] = 10
    # mn1.window.update()
    for col in ws.iter_cols(min_row=2, min_col=9, max_col=9, max_row=count):
        for cell in col:
            # mn1.progress_bar['value'] += 1
            # mn1.window.update()
            # if mn1.progress_bar['value'] > 100:
            #     mn1.progress_bar['value'] = 0
            snils = cell.value
            # print(snils)
        ...
        if (len(value) > 1) & (snils in dict_range):
            for i in value:
                for j in range(1, ws.max_column + 1):
                    ...
                    copy_cell(ws, i, j, ws_an, max_row1 + k, j)
                # print()
                # print(i)
                k += 1
                print(k)
...
def main_an2(down, up, folder):
    ...
    try:
        ...
        for col in ws.iter_cols(min_row=down, min_col=9, max_col=9, max_row=up):
            for cell in col:
                # mn1.progress_bar['value'] += 1
                # mn1.window.update()
                # if mn1.progress_bar['value'] > 100:
        ...
        wb.save(folder)
        wb.close()
        showinfo('Успешно', 'Анализ завершен')
        # mn1.progress_bar['value'] = 0
        # mn1.window.update()
    except:
        wb.save(folder)
